...
int32_t goal_position[2] = {0, 2048};

const uint8_t handler_index = 0;
const uint8_t handler_index2 = 1;
...
void setup() 
{
  ...
  result = dxl_wb.addSyncReadHandler(dxl_id[0], "Present_Velocity", &log);
  if (result == false)
  {
    Serial.println(log);
    Serial.println("Failed to add sync read handler2");
  }
}
...
{  
  ...
  do
  {
    ...
    result = dxl_wb.syncRead(handler_index2, &log);
    if (result == false)
    {
      Serial.println(log);
      Serial.println("Failed to sync read velocity");
    }


    result = dxl_wb.getSyncReadData(handler_index, &present_position[0], &log);
    result = dxl_wb.getSyncReadData(handler_index2, &present_velocity[0], &log);
    // result = dxl_wb.getSyncReadData(handler_index, &present_position[0], &log);
    if (result == false)
    {
      Serial.println(log);
  ...
  }while(abs(goal_position[0] - present_position[0]) > 5 && 
        abs(goal_position[1] - present_position[1]) > 5);
